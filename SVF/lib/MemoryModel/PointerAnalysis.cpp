    ...
    for (PointsTo::iterator ii = target.begin(), ie = target.end();
        ...
        if(ObjPN* objPN = dyn_cast<ObjPN>(pag->getPAGNode(*ii))) {
            ...
            if(obj->isFunction()) {
                ...
                bool retMatch = true;
                llvm::Type* funcReturnType = callee->getReturnType();
                llvm::Type* csReturnType = cs.getInstruction()->getType();

                if (funcReturnType->isVoidTy() && csReturnType) {
                    retMatch = false;
                }
                while (funcReturnType->isPointerTy()) {
                            funcReturnType = funcReturnType->getPointerElementType();
                }
                while (csReturnType && csReturnType->isPointerTy()) {
                            csReturnType = csReturnType->getPointerElementType();
                }
                // if(!funcReturnType && !csReturnType && csReturnType != funcReturnType){
                //     retMatch = false;
                // }

                if (llvm::StructType* funcretStTy = llvm::dyn_cast<llvm::StructType>(funcReturnType)) {
                    llvm::StructType* csretStTy = llvm::dyn_cast<llvm::StructType>(csReturnType);
                    if ( !csretStTy || !csretStTy->isLayoutIdentical(funcretStTy) ){
                        if ( !csretStTy ){
                            retMatch = false;
                        }else{
                            std::string csStName = csretStTy->getName();
                            std::string funcStName = funcretStTy->getName();
