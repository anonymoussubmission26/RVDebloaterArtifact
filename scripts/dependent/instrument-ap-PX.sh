...
echo "--- Adding Instrumentation to PX4 Bitcode ---"
...
cp ${PX_BIN}/${BCF_NAME} ${BCF_NAME}-uninstr.bc

make

# For copter
llvm-link ${BCF_NAME}-uninstr.bc req_funcs.bc -o ${BCF_NAME}-uninstr-linked.bc
# For plane
# llvm-link ${BCF_NAME}-uninstr.bc req_funcs_plane.bc -o ardu${VEHICLE}-uninstr-linked.bc
...
opt -load ./lib/AddDPFuncs.so -enable-new-pm=0 -dp-funcs ${BCF_NAME}-uninstr-linked.bc -o ${BCF_NAME}-instr-linked.bc
# opt -load ./lib/CallOverwrite.so -overwrite-call ${BCF_NAME}-uninstr-linked.bc -o ${BCF_NAME}-instr-linked.bc
# opt -load ./lib/AddTrack.so -enable-new-pm=0 -track-funcs ${BCF_NAME}-instr-linked.bc -o ${BCF_NAME}-instr-linked.bc
...
cp ${RVD_PATH}/instrumentation/${BCF_NAME}-instr-linked.bc ${PX_BIN}/${BCF_NAME}
