...
python3 $CALLGRAPH_DIR/convertSvfCfgToHumanReadable.py callgraph_final.dot > ./arduplane.svf.type.cfg
...
spa -condition-cfg "${BCF_NAME}" 2>&1 | tee ./arduplane.svf.conditional.direct.calls.cfg
spa -simple "${BCF_NAME}" 2>&1 | tee ./arduplane.svf.function.pointer.allocations.wglobal.cfg
