...
python3 $CALLGRAPH_DIR/convertSvfCfgToHumanReadable.py callgraph_final.dot > ./arducopter.svf.type.cfg
...
spa -condition-cfg "${BCF_NAME}" 2>&1 | tee ./arducopter.svf.conditional.direct.calls.cfg
spa -simple "${BCF_NAME}" 2>&1 | tee ./arducopter.svf.function.pointer.allocations.wglobal.cfg
echo "--- Finished Generating Other Callgraph With SVF ---"

echo "--- Cleaning Callgraphs ---" # The last one: mainfunc 
# python3 ${RVD_PATH}/python-utils/graphCleaner.py --fpanalysis --funcname mainfunc --output ./arducopter.svf.new.type.fp.wglobal.cfg --directgraphfile ./arducopter.svf.conditional.direct.calls.cfg --funcpointerfile ./arducopter.svf.function.pointer.allocations.wglobal.cfg -c ./arducopter.svf.type.cfg
python3 ${RVD_PATH}/python-utils/graphCleaner.py --fpanalysis --funcname main --output ./arducopter.svf.new.type.fp.wglobal.cfg --directgraphfile ./arducopter.svf.conditional.direct.calls.cfg --funcpointerfile ./arducopter.svf.function.pointer.allocations.wglobal.cfg -c ./arducopter.svf.type.cfg
# python3 ${RVD_PATH}/python-utils/graphCleaner.py --fpanalysis --funcname _ZN6CopterC2Ev --output ./arducopter.svf.new.type.fp.wglobal.cfg --directgraphfile ./arducopter.svf.conditional.direct.calls.cfg --funcpointerfile ./arducopter.svf.function.pointer.allocations.wglobal.cfg -c ./arducopter.svf.type.cfg
echo "--- Pruned Callgraph is Generated ---"


echo "--- Identifying Functions for Each Mode ---"
cd ${RVD_PATH}/scripts/callgraph
python3 preprocessFile.py --modetopropertymap mode.to.properties.json --outputpath ../../outputs/ --modereductionpath ../../stats/modereduction.stats --cfgpath ../../callgraphs/ 
