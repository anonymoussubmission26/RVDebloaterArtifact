...
def copy_mission_profiles(mission_fname):
    '''Copy profile files if they exist'''
    src_files = ["../profiles/access_violation_indirect.log", "../profiles/access_violation_direct.log"]
    dest_dir = "../profiles/"  # Adjust if needed

    for src in src_files:
        if os.path.exists(src):
            dest_file = os.path.join(dest_dir, f"{mission_fname}_{os.path.basename(src)}")
...
if __name__ == '__main__':
    ...
    for mission_type in ['copter']:
        # for fnum in range(1,7): #For Profiling (train)
        for fnum in range(7,10): #For Monitoring (test)
            ...
            copy_mission_profiles(mission_fname)
