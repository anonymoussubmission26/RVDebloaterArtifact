...
def run_mission(mavproxy,mission_fname):
    ...
    time_sleep(5)
    mavproxy.send('mode qloiter\n')
    # wait_mode(mav, ['QLOITER'])
    # if (init == 1):
...
if __name__ == '__main__':
    ...
    for mission_type in ['copter']:
        for fnum in range(1,11): #For Profiling (train)
        # for fnum in range(9,21): #For Monitoring (test)
